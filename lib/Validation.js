...
const Validation = options => handler => async (req, res) => {
    let url, provider, secret, branch, event // from options
    let sig, eventGit, sigDecrypted
    ...
    for (let i = 0; i < options.length; i++) {
        if (req.url == options[i].url) {
            ...
            branch = options[i].branch || 'master'
            event = options[i].event || 'push'
            break
        }
    }
    ...
    switch (provider) {
        case 'github':
            ...
            eventGit = req.headers['x-github-event']
    ...
    // check the branch
    if (branch == data.ref.split('/')[2] && event == eventGit) {
